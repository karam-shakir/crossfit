    ...
    <div className="fixed inset-0 z-50 bg-black/70 flex items-end sm:items-center justify-center p-0 sm:p-4">
      <div className="bg-gray-950 border border-gray-800 rounded-t-2xl sm:rounded-2xl w-full sm:max-w-lg max-h-[92vh] flex flex-col">
        <div className="flex items-center justify-between px-4 py-3 border-b border-gray-800 flex-shrink-0">
          <h2 className="font-bold text-white flex items-center gap-2">🧩 مكتبة التمارين</h2>
        ...
        <div className="overflow-y-auto flex-1 p-4 space-y-4">
          ...
          {!showAddForm ? (
            <button onClick={() => setShowAddForm(true)}
              className="w-full py-2.5 rounded-xl bg-lime-700 hover:bg-lime-600 text-white text-sm font-bold transition-colors">
              + إضافة تمرين جديد
            </button>
          ) : (
            <div className="bg-gray-900 rounded-xl border border-gray-800 p-3 space-y-2.5">
              <div className="grid grid-cols-2 gap-2">
                <input value={form.nameAr} onChange={e => setForm(f => ({ ...f, nameAr: e.target.value }))}
                  placeholder="الاسم بالعربي" className="bg-gray-800 border border-gray-700 rounded-lg px-3 py-2 text-white text-sm focus:outline-none focus:border-lime-500" />
                <input value={form.nameEn} onChange={e => setForm(f => ({ ...f, nameEn: e.target.value }))}
                  placeholder="Name in English" className="bg-gray-800 border border-gray-700 rounded-lg px-3 py-2 text-white text-sm focus:outline-none focus:border-lime-500" dir="ltr" />
              </div>
              <select value={form.category} onChange={e => setForm(f => ({ ...f, category: e.target.value }))}
                className="w-full bg-gray-800 border border-gray-700 rounded-lg px-3 py-2 text-white text-sm focus:outline-none focus:border-lime-500">
                ...
                <div className="flex flex-wrap gap-1.5">
                  {SECTIONS.map(s => (
                    <button key={s.key} type="button"
                      ...
                      className={`px-2.5 py-1 rounded-lg text-xs font-semibold transition-colors ${form.sections.includes(s.key) ? 'bg-lime-600 text-white' : 'bg-gray-800 text-gray-400 border border-gray-700'}`}>
                      {s.label}
                    </button>
                  ))}
                </div>
              ...
              <input value={form.youtube} onChange={e => setForm(f => ({ ...f, youtube: e.target.value }))}
                placeholder="رابط يوتيوب (اختياري)" className="w-full bg-gray-800 border border-gray-700 rounded-lg px-3 py-2 text-white text-sm focus:outline-none focus:border-lime-500" dir="ltr" />
              <input value={form.muscles} onChange={e => setForm(f => ({ ...f, muscles: e.target.value }))}
                placeholder="العضلات المستهدفة (اختياري)" className="w-full bg-gray-800 border border-gray-700 rounded-lg px-3 py-2 text-white text-sm focus:outline-none focus:border-lime-500" />
              {error && <div className="text-xs text-red-400">{error}</div>}
              <div className="flex gap-2">
                <button onClick={addExercise} disabled={saving}
                  className="flex-1 py-2 rounded-lg bg-lime-700 hover:bg-lime-600 disabled:opacity-60 text-white text-sm font-bold transition-colors">
              ...
              <div key={ex.id} className="bg-gray-900 rounded-xl border border-gray-800 p-3 space-y-2">
                  ...
                  <div className="min-w-0">
                    ...
                    <div className="flex flex-wrap gap-1 mt-1">
                      {(ex.sections || []).map((s: string) => (
                        <span key={s} className="text-[10px] bg-gray-800 text-gray-400 px-1.5 py-0.5 rounded-full">
                ...
                {ex.aiEligible ? (
                  <div className="flex items-center justify-between">
                    <span className="text-[11px] text-emerald-400 font-semibold">🤖 مؤهل للذكاء الاصطناعي</span>
