...
function NavLink({ href, iconKey, label, active, color }: {
  ...
  if (color === 'red') {
    ...
    inactiveClass = 'text-blue-400 hover:bg-gray-800 hover:text-blue-300';
  }
...
function Avatar({ name, role }: { name: string; role: string }) {
  ...
  const bg = role === 'admin' ? 'bg-amber-600' : 'bg-orange-500';
  return (
...
export default function Navbar({ member }: {
  ...
  return (
    <>
      ...
      {impersonationBanner && (
        <div
          className="fixed top-0 inset-x-0 z-50 bg-gray-900 border-b border-orange-500 text-white text-xs px-3 flex items-center justify-between gap-2"
          style={{ paddingTop: 'max(0.4rem, env(safe-area-inset-top))', paddingBottom: '0.4rem' }}
        >
          <button
            onClick={restoreAdmin}
            className="flex-shrink-0 bg-orange-500 text-white font-bold text-[11px] px-3 py-1.5 rounded-full hover:bg-orange-400 active:scale-95 transition-all whitespace-nowrap"
          >← الإدارة</button>
          <span className="truncate text-center flex-1 inline-flex items-center justify-center gap-1.5"><VenetianMask className="w-3.5 h-3.5 text-orange-400 flex-shrink-0"/><strong className="font-bold">{member.nameAr}</strong></span>
        </div>
      )}

      {/* ===== Desktop Sidebar ===== */}
      <aside className={`hidden lg:flex flex-col w-56 bg-gray-900 border-l border-gray-800 fixed right-0 bottom-0 z-40 ${impersonationBanner ? 'top-9' : 'top-0'}`}>
          ...
          <div className="flex items-center gap-3 bg-gray-800/60 rounded-xl px-3 py-2.5">
            ...
            <div className="min-w-0">
              ...
              <div className="text-xs text-gray-400 flex items-center gap-1 mt-0.5">
                {isAdmin
                  ? <><IcUserShield className="w-3 h-3 text-amber-400"/><span className="text-amber-400">مدير النظام</span></>
                  : canManageCrossfit
                  ? <><IcUserShield className="w-3 h-3 text-orange-400"/><span className="text-orange-400">مدرّب</span></>
              ...
              <Link href="/admin"
                className={`flex items-center gap-3 px-3 py-2 rounded-lg text-sm transition-all ${
                  pathname === '/admin'
                    ...
                    : 'text-amber-400 hover:bg-gray-800 hover:text-white'
                }`}>
      ...
      <nav className="lg:hidden fixed bottom-0 left-0 right-0 z-40 bg-gray-900 border-t border-gray-800 flex"
        ...
        {mobileBottomFiltered.map(l => {
          ...
          const activeColor = col === 'red' ? '#ef4444' : col === 'amber' ? '#d97706' : col === 'emerald' ? '#10b981' : col === 'blue' ? '#3b82f6' : '#f97316';
          return (
          ...
          <div
            ...
            <div className="grid grid-cols-2 gap-2.5">
              {mobileMoreFiltered.map(l => {
                ...
                if (col === 'blue')    { activeBg = '#2563eb'; inactiveBg = '#eff6ff'; inactiveText = '#1e40af'; }
              ...
              {canManageCrossfit && (
                <Link href="/admin" onClick={() => setOpen(false)}
                  style={{
                    ...
                    backgroundColor: pathname === '/admin' ? '#d97706' : '#fffbeb',
                    color: pathname === '/admin' ? '#ffffff' : '#92400e',
                    textDecoration: 'none',
                  }}>
