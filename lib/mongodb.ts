...
async function ensureIndexes(db: Db): Promise<void> {
  ...
  await Promise.all([
    ...
    db.collection('running_week_meta').createIndex({ memberId: 1, weekStartDate: -1 }).catch(() => {}),
    db.collection('wod_cycle_meta').createIndex({ weekStartDate: -1 }, { unique: true }).catch(() => {}),
