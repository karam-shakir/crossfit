...
const SESSION_TYPES = [
  {
    ...
    labelEn: 'Strength',
    icon: '💪',
    desc: 'بناء القوة المطلقة بالوزن الذاتي',
    color: 'border-blue-500 bg-blue-900/20 text-blue-300',
    activeColor: 'border-blue-400 bg-blue-500/30 text-white',
  },
  {
    id: 'skills',
    label: 'مهارات',
    labelEn: 'Skills',
    icon: '🤸',
    desc: 'وقوف على يدين، ليفر، بلانش',
    color: 'border-purple-500 bg-purple-900/20 text-purple-300',
    activeColor: 'border-purple-400 bg-purple-500/30 text-white',
  },
  {
    id: 'endurance',
    label: 'تحمل',
    labelEn: 'Endurance',
    icon: '🔄',
    desc: 'تكرارات عالية وكثافة قلبية',
    color: 'border-green-500 bg-green-900/20 text-green-300',
    activeColor: 'border-green-400 bg-green-500/30 text-white',
  },
  {
    id: 'mixed',
    label: 'مختلط',
    labelEn: 'Mixed',
    icon: '⚡',
    desc: 'قوة + مهارات + تحمل',
    color: 'border-orange-500 bg-orange-900/20 text-orange-300',
    activeColor: 'border-orange-400 bg-orange-500/30 text-white',
  },
  {
    id: 'hiit',
    label: 'HIIT',
    labelEn: 'HIIT',
    icon: '🔥',
    desc: 'تدريب متقطع عالي الكثافة',
    color: 'border-red-500 bg-red-900/20 text-red-300',
    activeColor: 'border-red-400 bg-red-500/30 text-white',
  },
  {
    id: 'crossfit_mix',
    label: 'CrossFit Mix',
    labelEn: 'CrossFit Mix',
    icon: '🏋️',
    desc: 'كاليسثينكس + حديد CrossFit',
    color: 'border-yellow-500 bg-yellow-900/20 text-yellow-300',
    activeColor: 'border-yellow-400 bg-yellow-500/30 text-white',
  },
];

const FOCUS_OPTIONS = [
  'كامل الجسم', 'الجزء العلوي', 'الجزء السفلي',
  'القلب والكور', 'مهارات الجمناستيكس', 'الكتفين والضغط',
  'الظهر والسحب', 'تدريب الحلقات',
];
        ...
        <div className="px-4 pb-4 space-y-2 border-t border-gray-700 pt-3">
          ...
          {ex.scaling && (
            <div className="mt-2 grid grid-cols-2 gap-2">
              {ex.scaling.easier && (
                <div className="bg-green-900/20 border border-green-800/50 rounded-lg px-2 py-1.5">
                  <div className="text-xs text-green-400 font-medium mb-0.5">↓ أسهل</div>
                  <div className="text-xs text-gray-300">{ex.scaling.easier}</div>
                </div>
              )}
              {ex.scaling.harder && (
                <div className="bg-red-900/20 border border-red-800/50 rounded-lg px-2 py-1.5">
                  <div className="text-xs text-red-400 font-medium mb-0.5">↑ أصعب</div>
                  <div className="text-xs text-gray-300">{ex.scaling.harder}</div>
                </div>
              )}
            </div>
          )}
          {ex.progression && (
            <div className="text-xs bg-purple-900/20 border border-purple-800/40 rounded-lg px-2 py-1.5 text-purple-300">
              🚀 التطور: {ex.progression}
            </div>
          )}
          {ex.regression && (
            <div className="text-xs bg-blue-900/20 border border-blue-800/40 rounded-lg px-2 py-1.5 text-blue-300">
              📉 البديل: {ex.regression}
            </div>
          )}
        </div>
      )}
...
}: { title: string; icon: string; colorClass: string; children: React.ReactNode }) {
  return (
...
export default function CalisthenicsClient({ member }: { member: any }) {
    ...
    try {
      const res  = await fetch('/api/calisthenics/generate', {
        ...
        body:    JSON.stringify({
          ...
          sessionType,
          focus,
          mixCrossfit: sessionType === 'crossfit_mix',
        }),
      });
  ...
  function buildWhatsApp() {
    ...
    const lines: string[] = [
      `🤸 *${session.title}*`,
      `📅 ${date}  |  🎯 ${session.focus}  |  ⚡ ${session.difficulty}`,
      `⏱ المدة: ${session.totalDuration} دقيقة`,
      '',
    ];
    if (session.metcon) {
      lines.push(`🔥 *الميتكون — ${session.metcon.format}*`);
      if (session.metcon.timecap) lines.push(`تايم كاب: ${session.metcon.timecap} دقيقة`);
      (session.metcon.exercises || []).forEach((e: any) => {
        lines.push(`• ${e.reps} ${e.name}`);
      });
      lines.push('');
    }
  ...
  return (
    <div className="min-h-screen flex">
      ...
      <main className="flex-1 lg:mr-56 pb-24 lg:pb-0">
        <div className="max-w-2xl mx-auto px-4 py-6 space-y-6">
          ...
          <div className="bg-gray-900 rounded-2xl border border-gray-800 p-5 space-y-5">
              ...
              <div className="grid grid-cols-2 gap-2 sm:grid-cols-3">
                {SESSION_TYPES.map(t => (
                  <button
                    ...
                    className={`flex flex-col gap-1 p-3 rounded-xl border text-right transition-all ${
                      sessionType === t.id ? t.activeColor : t.color + ' opacity-70 hover:opacity-100'
                    }`}
                  >
            ...
            {error && (
              <div className="bg-red-900/30 border border-red-700 rounded-xl p-3 text-sm text-red-300 text-center">
                ⚠️ {error}
              </div>
            )}
          </div>
            ...
            <div className="space-y-4">
              ...
              <div className="bg-gradient-to-l from-emerald-900/30 to-gray-900 rounded-2xl border border-emerald-700/40 p-5">
                <div className="flex items-start justify-between gap-3 mb-3">
                  <div>
                    ...
                    <div className="flex flex-wrap gap-2 mt-2">
                      <span className="text-xs bg-emerald-500/20 text-emerald-300 px-2 py-1 rounded-full border border-emerald-700">
                        🤸 {session.sessionType}
                      </span>
                      <span className="text-xs text-gray-400">⚡ {session.difficulty}</span>
                      <span className="text-xs text-gray-400">🎯 {session.focus}</span>
                      {session.totalDuration && (
                        <span className="text-xs text-gray-400">⏱ {session.totalDuration} دقيقة</span>
                      )}
                    </div>
                  </div>
                  <button
                    ...
                    className="flex-shrink-0 bg-green-600 hover:bg-green-500 text-white px-3 py-1.5 rounded-xl text-xs font-bold flex items-center gap-1.5 transition-colors"
                  >
                ...
                {session.equipment && session.equipment.length > 0 && (
                  <div className="flex flex-wrap gap-2">
                    {session.equipment.map((eq: string, i: number) => (
              ...
              {session.skillWork?.exercises?.length > 0 && (
                <Section title={`${session.skillWork.title || 'العمل على المهارة'} — ${session.skillWork.duration} دقيقة`} icon="🤸" colorClass="border-purple-700/40 bg-purple-900/10">
                  {session.skillWork.exercises.map((ex: any, i: number) => (
                    <ExerciseRow key={i} ex={ex} index={i}/>
                  ))}
                </Section>
              )}
              ...
              {session.mainWork?.map((block: any, bi: number) => (
                <Section
                  key={bi}
                  title={`${block.block}${block.duration ? ` — ${block.duration} دقيقة` : ''}`}
                  icon="💪"
                  colorClass="border-blue-700/40 bg-blue-900/10"
                >
                  {/* نوع الكتلة */}
                  {block.type && (
                    <div className="text-xs text-blue-400 font-medium mb-2 uppercase tracking-wider">{block.type}</div>
                  )}
                  {block.exercises?.map((ex: any, ei: number) => (
                    <ExerciseRow key={ei} ex={ex} index={ei}/>
                  ))}
                </Section>
              ))}
                  ...
                  <div className="px-4 py-3 border-b border-orange-700/30 flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="font-bold text-sm text-orange-300">
                        {session.metcon.title || 'الميتكون'}
                      </span>
                    </div>
                    <div className="flex items-center gap-2">
                      <span className="text-xs bg-orange-500/20 text-orange-300 px-2 py-0.5 rounded-full border border-orange-700">
                        {session.metcon.format}
                      </span>
                      {session.metcon.timecap && (
                        <span className="text-xs text-gray-400">⏱ {session.metcon.timecap} دقيقة</span>
                      )}
                      {session.metcon.rounds && (
                        <span className="text-xs text-gray-400">🔄 {session.metcon.rounds} جولات</span>
                      )}
                    </div>
                  </div>
                    ...
                    {session.metcon.exercises?.map((ex: any, i: number) => (
                      <div key={i} className="flex items-center gap-3 py-2 border-b border-gray-800 last:border-0">
                        ...
                        <div className="flex-1">
                          ...
                          {ex.nameEn && <span className="text-gray-500 text-xs mr-2">{ex.nameEn}</span>}
                        </div>
                        {ex.reps && (
                          <span className="text-orange-300 font-bold text-sm">{ex.reps}</span>
                        )}
                      </div>
                    ))}
                    {session.metcon.scoreType && (
                      <div className="text-xs text-gray-500 mt-2">📊 النتيجة: {session.metcon.scoreType}</div>
                    )}
                  </div>
                </div>
              )}
                  ...
                  {session.cooldown.stretches.map((s: any, i: number) => (
                    <div key={i} className="flex items-center gap-3 py-2 border-b border-gray-800 last:border-0">
                      ...
                      <div className="flex-1">
                        ...
                        {s.focus && <span className="text-gray-500 text-xs mr-2">— {s.focus}</span>}
                      </div>
              ...
              <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                {session.weeklyPlacement && (
                  <div className="bg-gray-900 rounded-xl border border-gray-800 p-4">
                    <div className="text-xs text-gray-500 mb-1.5">📅 الترتيب في الأسبوع</div>
                    <div className="text-sm text-gray-200">{session.weeklyPlacement}</div>
                  </div>
                )}
                {session.progressionPath && (
                  <div className="bg-gray-900 rounded-xl border border-gray-800 p-4">
                    <div className="text-xs text-gray-500 mb-1.5">🚀 مسار التطور</div>
                    <div className="text-sm text-gray-200">{session.progressionPath}</div>
                  </div>
                )}
                {session.nutritionTips && (
                  <div className="bg-amber-900/20 border border-amber-700/30 rounded-xl p-4 sm:col-span-2">
                    <div className="text-xs text-amber-400 mb-1.5">🥗 التغذية</div>
                    <div className="text-sm text-gray-300">{session.nutritionTips}</div>
                  </div>
                )}
                {session.coachNote && (
                  <div className="bg-emerald-900/20 border border-emerald-700/30 rounded-xl p-4 sm:col-span-2">
                    <div className="text-xs text-emerald-400 mb-1.5">💬 ملاحظة المدرب</div>
                    <div className="text-sm text-gray-300">{session.coachNote}</div>
                  </div>
                )}
              </div>
