...
import { Trophy, Trash2, TrendingUp, Play, ChevronUp } from 'lucide-react';
        ...
        <div className="max-w-2xl mx-auto px-4 pt-safe pb-6 space-y-6">
          ...
          <div className="flex items-center justify-between">
            <h1 className="text-xl font-bold text-white flex items-center gap-2">
              <Trophy className="w-5 h-5 text-amber-400" /> أرقامي الشخصية
            </h1>
                ...
                <button onClick={addPR} disabled={!form.exerciseId || !form.value || saving}
                  ...
                  {saving ? 'جاري الحفظ...' : (
                    <span className="inline-flex items-center justify-center gap-1">
                      حفظ الرقم <Trophy className="w-4 h-4 text-amber-400" />
                    </span>
                  )}
                </button>
          ...
          {loading ? (
            ...
            <div className="bg-gray-900 rounded-2xl p-8 text-center border border-gray-800">
              <Trophy className="w-6 h-6 text-gray-600 mb-4 mx-auto" />
                    ...
                    <div className="flex gap-2 flex-shrink-0">
                      {pr.exercise?.youtube && (
                        <a href={pr.exercise.youtube} target="_blank" rel="noopener noreferrer"
                          className="w-8 h-8 rounded-lg bg-red-600 hover:bg-red-500 flex items-center justify-center text-xs transition-colors"><Play className="w-4 h-4" /></a>
                      )}
                      ...
                      <button
                        ...
                        {expandedChart === pr.id ? (
                          <span className="inline-flex items-center gap-1">
                            <ChevronUp className="w-4 h-4" /> إخفاء الرسم البياني
                          </span>
                        ) : (
                          <span className="inline-flex items-center gap-1">
                            <TrendingUp className="w-4 h-4 text-orange-500" /> عرض التطور ({pr.history.length} تسجيل)
                          </span>
                        )}
                      </button>
