...
export async function POST(req: NextRequest) {
  ...
  // (Vercel Runtime Timeout) لأن التوليد تجاوز الحد الأقصى 300 ثانية لمدة الدالة — الحساب الفعلي:
  // اختبار حي سابق بميزانية توكنز أقل من الحالية استهلك 337 ثانية لست أيام فقط قبل نفاد الميزانية،
  // أي أن ست أيام وحدها كانت أصلاً عند حافة 300 ثانية حتى قبل زيادة حجم البرومت (محظورات دمج
  // الحركات + دوران نوع التحفيز أضيفا لاحقاً). حتى تُفعَّل خطة Vercel Fluid Compute (تسمح برفع
  // maxDuration فوق 300 بأمان)، نمنع الطلب من الأساس بدل تركه يفشل بعد دقائق من الانتظار.
  const MAX_SAFE_WEEKLY_DAYS = 5;
