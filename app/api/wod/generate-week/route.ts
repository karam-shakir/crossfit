...
// يمنح Vercel وقتاً كافياً لتوليد استجابات طويلة قبل قطع الاتصال — رُفعت من 300 إلى 800
// بعد رصد اختبار حي لـ7 أيام استغرق 337 ثانية بمعدل ~83 توكن/ثانية وهو ناتج قديم كان مقطوعاً أصلاً؛
// رفع maxTokens أدناه (لإصلاح القطع) يعني وقت توليد أطول أيضاً، فالحد القديم كان سيصبح عنق زجاجة جديداً.
// ⚠️ 800 ثانية يتطلب خطة Vercel Pro مع تفعيل Fluid Compute — إن لم تكن مفعّلة، فيرسل يقيّد المدة تلقائياً
// عند الحد الفعلي لخطتك (300 عادة على Pro القياسي) بغض النظر عن هذا الرقم، فتحقق من إعدادات المشروع.
...
أرجع JSON فقط، بدون أي نص قبله أو بعده.`;
  ...
  // الحد الأقصى للتوكنز مرتبط بحجم المحتوى الفعلي لكل يوم. هذه القيمة (4000 توكن/يوم) نفسها كانت
  // مصححة سابقاً من قيمة أقدم (1000/يوم) بعد رصد قطع عند 7 أيام — لكن بعد ترحيل بنية البلوكات
  // (٣ بلوكات إحماء + executionNote + مستويات "levels" الأربعة الآن إلزامية حتى في الأكسسوار) صار
  // كل يوم أثقل بكثير من التقدير القديم. اختبار حي فعلي لـ7 أيام مع الصيغة القديمة
  // (min(32000, max(16000, 7×4000)) = 28000) أرجع usage.output_tokens=28000 بالضبط مع
  // stop_reason="max_tokens" — قطع مؤكد: 6 من 7 أيام فقط ظهرت، واليوم السابع وweekSummary/
  // recoveryTips/nutritionNote/progressionNote (تُقرأ لاحقاً عند توليد الأسبوع القادم) فُقدت بالكامل.
  // الصيغة الجديدة: ميزانية أعلى بكثير لكل يوم + هامش ثابت للحقول الختامية، وسقف أعلى يستفيد من
  // أن Claude Opus 5 يدعم حتى 128000 توكن إخراج فعلياً (كنّا بعيدين جداً عن هذا الحد فعلياً).
  ...
  try {
    ...
    // سجل تشخيصي دائم — أبقيناه بعد اكتشاف قطع فعلي عند 7 أيام (usage.output_tokens == maxTokens
    // بالضبط مع stop_reason="max_tokens") لمراقبة أي انحراف مشابه مستقبلاً إن كبرت البنية مجدداً
