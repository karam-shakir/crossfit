...
const RUN_THEME: Record<string, { accent: string; badge: string; icon: string; label: string }> = {
  Easy:      { accent: 'bg-emerald-500', badge: 'bg-emerald-100 text-emerald-700 border border-emerald-200', icon: '🌿', label: 'جري سهل' },
  Tempo:     { accent: 'bg-orange-500',  badge: 'bg-orange-100 text-orange-700 border border-orange-200',    icon: '🔥', label: 'إيقاعي' },
  Intervals: { accent: 'bg-red-500',     badge: 'bg-red-100 text-red-700 border border-red-200',             icon: '⚡', label: 'تكرارات' },
  Long:      { accent: 'bg-indigo-500',  badge: 'bg-indigo-100 text-indigo-700 border border-indigo-200',    icon: '🛣️', label: 'جري طويل' },
  Recovery:  { accent: 'bg-teal-400',    badge: 'bg-teal-100 text-teal-700 border border-teal-200',          icon: '💧', label: 'استرداد' },
  Hills:     { accent: 'bg-amber-500',   badge: 'bg-amber-100 text-amber-700 border border-amber-200',       icon: '⛰️', label: 'تلال' },
  Fartlek:   { accent: 'bg-violet-500',  badge: 'bg-violet-100 text-violet-700 border border-violet-200',    icon: '🎲', label: 'فارتلك' },
  Cross:     { accent: 'bg-sky-500',     badge: 'bg-sky-100 text-sky-700 border border-sky-200',             icon: '🚴', label: 'تدريب متقاطع' },
...
function SessionCard({ s, isToday }: { s: any; isToday: boolean }) {
  ...
  return (
    <div className={`rounded-2xl overflow-hidden shadow-sm border border-slate-200 bg-white ${isToday ? 'ring-2 ring-cyan-400 ring-offset-1' : ''}`}>
      ...
      {isToday && (
        <div className="bg-cyan-600 text-white text-sm font-bold text-center py-2 tracking-widest">
          ✨ جلسة اليوم
        </div>
      )}
        ...
        <main className="flex-1 lg:mr-56 flex items-center justify-center px-4">
          <div className="text-center space-y-5 max-w-sm">
            ...
            <Link href="/running/profile"
              className="inline-block bg-cyan-600 hover:bg-cyan-700 text-white font-bold px-8 py-4 rounded-2xl transition-colors text-base shadow-lg shadow-cyan-200">
            ...
            {sessions.length > 0 && (
              <div className="mt-4 pt-3 border-t border-white/20 grid grid-cols-3 gap-2">
                <div className="text-center">
                  ...
                  <div className="text-[11px] mt-0.5" style={{ color: 'rgba(255,255,255,0.70)' }}>أيام جري</div>
                </div>
                <div className="text-center border-x border-white/20">
                  <div className="text-2xl font-extrabold text-white">{totalKm.toFixed(0)}</div>
                  <div className="text-[11px] mt-0.5" style={{ color: 'rgba(255,255,255,0.70)' }}>كم / أسبوع</div>
                </div>
                <div className="text-center">
          ...
          {sessions.length === 0 ? (
            <div className="text-center py-20 space-y-4">
              ...
              <p className="text-slate-500 text-sm leading-relaxed max-w-xs mx-auto">
                بروفايلك جاهز ✅ — انتظر المدرب ليولّد لك برنامج الجري الأسبوعي المخصص
              </p>
            </div>
          ) : (
            <>
