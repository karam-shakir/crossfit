...
import { Ruler, Scale, Droplet, Dumbbell, Target, Move, Bird, BicepsFlexed, Footprints, TrendingUp } from 'lucide-react';
...
const FIELDS = [
  { key: 'weight',   label: 'الوزن (كجم)',     icon: Scale,       lowerIsBetter: true  },
  { key: 'height',   label: 'الطول (سم)',       icon: Ruler,       lowerIsBetter: false },
  { key: 'bodyFat',  label: 'نسبة الدهون (%)',  icon: Droplet,     lowerIsBetter: true  },
  { key: 'chest',    label: 'الصدر (سم)',        icon: Dumbbell,    lowerIsBetter: false },
  { key: 'waist',    label: 'الخصر (سم)',        icon: Target,      lowerIsBetter: true  },
  { key: 'hips',     label: 'الأرداف (سم)',      icon: Move,        lowerIsBetter: null  },
  { key: 'shoulders',label: 'الأكتاف (سم)',      icon: Bird,        lowerIsBetter: false },
  { key: 'arm',      label: 'الذراع (سم)',        icon: BicepsFlexed, lowerIsBetter: false },
  { key: 'thigh',    label: 'الفخذ (سم)',         icon: Footprints,  lowerIsBetter: false },
];
        ...
        <div className="max-w-2xl mx-auto px-4 pt-safe pb-6 space-y-5">
          ...
          <div className="flex items-center justify-between">
            <h1 className="text-xl font-bold text-white flex items-center gap-2">
              <Ruler className="w-5 h-5 text-orange-500" /> القياسات الجسدية
            </h1>
          ...
          {showForm && (
            <div className="bg-gray-900 rounded-2xl p-4 border border-orange-700 space-y-3">
              ...
              <div className="grid grid-cols-2 gap-3">
                {FIELDS.map(f => (
                  <div key={f.key}>
                    <label className="text-xs text-gray-400 mb-1 flex items-center gap-1"><f.icon className="w-4 h-4" /> {f.label}</label>
                ...
                <button onClick={save} disabled={saving}
                  ...
                  {saving ? 'جاري الحفظ...' : (
                    <span className="inline-flex items-center justify-center gap-1">
                      حفظ القياسات <Ruler className="w-4 h-4" />
                    </span>
                  )}
                </button>
          ...
          {loading ? (
            ...
            <div className="bg-gray-900 rounded-2xl p-8 text-center border border-gray-800">
              <Ruler className="w-6 h-6 text-gray-600 mb-4 mx-auto" />
                  ...
                  {FIELDS.filter(f => latest[f.key]).map((f, i) => {
                    ...
                    return (
                      <div key={f.key} className={`p-3 text-center ${i % 3 !== 2 ? 'border-l border-gray-800' : ''} border-b border-gray-800`}>
                        ...
                        <div className="text-xs text-gray-500 flex items-center justify-center gap-1"><f.icon className="w-4 h-4" /> {f.label.split(' ')[0]}</div>
                        {d && (
              ...
              {records.length >= 2 && (
                <div className="bg-gray-900 rounded-2xl border border-gray-800 p-4 space-y-3">
                  <h2 className="font-semibold text-white flex items-center gap-2">
                    <TrendingUp className="w-4 h-4 text-orange-500" /> الرسم البياني للتقدم
                  </h2>
                  ...
                  <div className="flex gap-2 flex-wrap">
                    {FIELDS.filter(f => records.some(r => r[f.key] != null)).map(f => (
                      <button key={f.key} onClick={() => setChartField(f.key)}
                        ...
                        <span className="inline-flex items-center gap-1"><f.icon className="w-4 h-4" /> {f.label.split(' ')[0]}</span>
                      </button>
                    ))}
                  </div>
                    ...
                    <div key={r.id} className="bg-gray-900 rounded-xl border border-gray-800 p-4">
                      ...
                      <div className="flex flex-wrap gap-3">
                        {FIELDS.filter(f => r[f.key]).map(f => (
                          <span key={f.key} className="text-xs bg-gray-800 px-2 py-1 rounded-lg text-gray-300 inline-flex items-center gap-1">
                            <f.icon className="w-4 h-4" /> {r[f.key]}
                          </span>
                        ))}
                      </div>
                    </div>
